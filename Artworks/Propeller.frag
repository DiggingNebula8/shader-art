void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float ringRadius = 0.4;  // Adjustable radius of the ring
    ...
    vec3 ringColor = alpha * vec3(.5, 0.0, 0.0);
    
    // Control Parameters
    float speed = 110.0; 
    float blades = 3.0; // Adjustable blade count
    float blur = 0.5; // Blade softness
    float innerFade = 0.07; // Center fade-out
    float outerFade = .01; // Outer edge fade-in
    vec3 bladeColor = vec3(1.0, 1.0, 1.); // Blade main color
